...
| 📊 **SQLite Ready** | Ready to use `bun:sqlite` for database (optional) |
...
| Status | Feature |
|--------|---------|
| 🔄 In Progress | SQLite database integration via `bun:sqlite` |
...
> Multi-session support is on the roadmap. For now, each instance runs with a single number. You can duplicate the project folder as a workaround.
