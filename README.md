...
| 🗄️ **SQLite Auth State** | Secure session storage using SQLite with AES-256-GCM encryption |
...
├── package.json
├── database/                  # Session & shared databases (auto-generated)
│   └── sessions/              # SQLite session files per bot instance
├── logs/                      # PM2 log files
...
└── src/
    ...
    │   ├── encryption.js      # Credential encryption (AES-256-GCM)
    │   ├── database.js        # Database initialization (Bun + Node.js)
    │   └── constants.js       # Auth constants & shared key types
    ├── core/
    ...
    │   ├── jadibot.js         # Clone bot process manager
    │   └── worker.js          # Isolated clone bot worker
...
export const init = {
    session: "./database/sessions",  // Session folder
...
const handler = async (m, { reply, text, isOwner }) => {
...
const handler = async (m, { conn, args, isGroup, isAdmin, botAdmin, reply }) => {
    ...
    await reply(`✅ Successfully kicked ${users.length} user(s)!`);
};
...
Asuma supports creating isolated bot instances (jadibot) that run in separate child processes.
...
- Each clone bot runs in its own isolated process (`child_process.fork`)
- Uses SQLite for session storage — separate database per clone in `database/sessions/`
- Auto-restart on crash (up to 5 retries with 10s cooldown)
- Clone bots do not share event loop with the main bot — no performance interference
...
**Requirements:**
- **Node.js:** Uses `better-sqlite3` (optional dependency — install fails gracefully)
- **Bun:** Uses built-in `bun:sqlite` — no additional installation needed

---

## 📦 Built-in Methods

After the bot is connected, the socket object has additional methods:
...
| Session Storage | SQLite with AES-256-GCM encryption |
| Process Manager | PM2 |
| Logger | pino + chalk |
| Media Processing | fluent-ffmpeg, jimp |
| HTTP Client | axios |
...
- ✅ SQLite Auth State with AES-256-GCM encryption
- ✅ Clone Bot / Jadibot system (isolated child processes)
- ✅ PM2 ecosystem config with auto-detect runtime (Bun/Node.js)
...
Yes! Use the `.jadibot` command to create isolated clone bot instances. Each clone runs in its own child process with its own SQLite session in `database/sessions/`.
...
- Delete the `database/sessions/` folder and reconnect
...
- Ensure the number is not already registered as a clone (use `.listjadibot`)
- Check PM2 logs: `npm run pm2:logs`

**Q: Clone bot disconnected and won't reconnect**

The system will auto-restart up to 5 times with a 10 second cooldown. If still failing, delete the session manually from `database/sessions/session_clone_<nomor>.db` and recreate with `.jadibot`.

**Q: Main bot becomes slow when clones are running**

Clone bots run in isolated child processes with their own event loop and should not affect main bot performance. If you experience lag, check your server's CPU/RAM usage.

**Q: `better-sqlite3` error on Node.js**

If you see a GLIBC version error, try:
```bash
npm rebuild better-sqlite3
# or
npm install better-sqlite3 --build-from-source
```
Alternatively, switch to Bun which uses the built-in `bun:sqlite` with no native dependencies.
...
Open an [Issue](https://github.com/asuma-labs/asuma/issues) and include:
...
Open an [Issue](https://github.com/asuma-labs/asuma/issues) with the `enhancement` label and describe your idea clearly.

---

## 📜 License

Released under the [CC BY-NC 4.0](https://creativecommons.org/licenses/by-nc/4.0/) (Creative Commons Attribution-NonCommercial 4.0 International) license.
